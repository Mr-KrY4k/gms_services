...
final class Storage {
  ...
  static Future<void> _ensureHiveInitialized() async {
    ...
    try {
      ...
      GmsLogger.debug('Storage: Hive инициализирован по пути: $path');
    } catch (e, st) {
      GmsLogger.error(
  ...
  Future<void> init() async {
    if (_isInitialized) {
      GmsLogger.debug('Storage[$boxName]: уже инициализирован');
      return;
    }

    if (_isPending) {
      GmsLogger.debug('Storage[$boxName]: инициализация уже выполняется');
      return;
    }

    try {
      _isPending = true;
      GmsLogger.debug('Storage[$boxName]: начало инициализации');
      ...
      GmsLogger.debug('Storage[$boxName]: бокс открыт, записей: $count');
      GmsLogger.debug('Storage[$boxName]: успешно инициализирован');
    } catch (e, st) {
  ...
  Future<void> save(String key, Map<String, dynamic> data) async {
    ...
    try {
      ...
      GmsLogger.debug('Storage[$boxName]: данные сохранены: $key');
    } catch (e, st) {
      GmsLogger.error(
  ...
  Map<String, dynamic>? get(String key) {
    ...
    try {
      ...
      if (data != null) {
        GmsLogger.debug('Storage[$boxName]: данные получены: $key');
        return Map<String, dynamic>.from(data);
      }
      GmsLogger.debug('Storage[$boxName]: данные не найдены: $key');
      return null;
    } catch (e, st) {
      GmsLogger.error(
  ...
  List<Map<String, dynamic>> getAll() {
    ...
    try {
      ...
      final result = allData
          ...
          .toList();
      GmsLogger.debug('Storage[$boxName]: получено ${result.length} записей');
      return result;
    } catch (e, st) {
      GmsLogger.error(
  ...
  List<String> getAllKeys() {
    ...
    try {
      ...
      GmsLogger.debug('Storage[$boxName]: получено ${keys.length} ключей');
      return keys;
    } catch (e, st) {
      GmsLogger.error(
  ...
  Future<bool> delete(String key) async {
    ...
    try {
      if (_box!.containsKey(key)) {
        ...
        GmsLogger.debug('Storage[$boxName]: данные удалены: $key');
        return true;
      }
      GmsLogger.debug(
  ...
  Future<void> clear() async {
    ...
    try {
      await _box!.clear();
      GmsLogger.debug('Storage[$boxName]: все данные удалены');
    } catch (e, st) {
      GmsLogger.error(
